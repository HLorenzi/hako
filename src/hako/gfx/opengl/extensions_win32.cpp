	...
	namespace OpenGL
	{
		...
		TFunction win32_get_function(const char* name)
		{
			...
			if (!ret)
			{
				...
				HAKO_ERROR("missing opengl extension function");
				return nullptr;
			}
			return ret;
		}
		...
		void win32_load_extensions()
		{
			...
			HAKO_WGL_GETFUNCTION(glDeleteBuffers,            PTR_GLDELETEBUFFERS);
			...
			HAKO_WGL_GETFUNCTION(glDeleteProgram,            PTR_GLDELETEPROGRAM);
			HAKO_WGL_GETFUNCTION(glDeleteShader,             PTR_GLDELETESHADER);
