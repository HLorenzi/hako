/*

FIXME: Unfinished!

*/


#include <hako/helper/debug_font.h>
#include <hako/util/debug.h>
#include <hako/engine.h>


unsigned char debug_font_pixels[] =
{
	
};


Hako::Helper::DebugFont::DebugFont()
{
#ifdef HAKO_BUILD_DEBUG
	this->initialized = false;
#endif
}


Hako::Helper::DebugFont::~DebugFont()
{
	HAKO_ASSERT(!this->initialized, "possible resource leak; destroy() was not called");
}


void Hako::Helper::DebugFont::init(Hako::Gfx::RenderOperation* renderop, Hako::Gfx::FrameBuffer* color_buffer)
{
    HAKO_UNUSED(renderop);

	unsigned char* decompressed_pixels =
		(unsigned char*)Hako::singleton_allocator().allocate.call(256 * 64 * 4, 32);

	for (unsigned int i = 0; i < 256 * 64 / 8; i++)
	{
		unsigned char value = debug_font_pixels[i];
		for (unsigned int j = 0; j < 8; j++)
		{
			unsigned char color = ((value & (0x80 >> j)) != 0 ? 255 : 128);
			decompressed_pixels[(i * 8 + j) * 4 + 0] = color;
			decompressed_pixels[(i * 8 + j) * 4 + 1] = color;
			decompressed_pixels[(i * 8 + j) * 4 + 2] = color;
			decompressed_pixels[(i * 8 + j) * 4 + 3] = 255;
		}
	}

	this->texture.init();
	this->texture.set_2d(256, 64, 1);
	this->texture.set_format(Hako::Gfx::TextureFormat::LinearRGBA8);
	this->texture.set_filter(Hako::Gfx::TextureFilter::Nearest);
	this->texture.set_data(0, 0, decompressed_pixels);
	this->texture.apply();

	Hako::singleton_allocator().free.call(decompressed_pixels);


	this->scene.init();
	this->translate_node = this->scene.add_translation(nullptr);
	this->renderer_node  = this->scene.add_renderer(&this->translate_node);

	this->camera.init();
	this->camera.set_projection_ortho(0, 960, 0, 544, -10, 10);
	this->camera.set_lookat(Hako::Math::Vector3::make(0, 0, 0), Hako::Math::Vector3::make(0, 0, 1), Hako::Math::Vector3::make(0, -1, 0));

	this->renderop.init();
	this->renderop.set_scene(&this->camera, &this->scene, 0);
	this->renderop.set_color_buffer(0, color_buffer);

	Hako::singleton_gfx()->add_operation(&this->renderop);

#ifdef HAKO_BUILD_DEBUG
	this->initialized = true;
#endif
}


void Hako::Helper::DebugFont::destroy()
{
	HAKO_ASSERT(this->initialized, "init() was not called");
	Hako::singleton_gfx()->remove_operation(&this->renderop);
	this->renderop   .destroy();
	this->material   .destroy();
	this->properties .destroy();
	this->texture    .destroy();
	this->mesh       .destroy();
#ifdef HAKO_BUILD_DEBUG
	this->initialized = false;
#endif
}


void Hako::Helper::DebugFont::set_text(const char* str)
{
	this->str = str;
}


void Hako::Helper::DebugFont::set_position(float x, float y)
{
	this->x = x;
	this->y = y;
}
